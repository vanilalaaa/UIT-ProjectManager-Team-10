...
export default function TeacherProjectDetail() {
  ...
  useEffect(() => {
    ...
    getProjectById(projectId ?? '')
      .then(async (foundProject) => {
        ...
        if (foundProject?.groupId) {
          const group = await getGroupById(foundProject.groupId).catch(() => null)
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 animate-fade-in">
      <ProjectDetailCard project={project} showEditButton={false} />
          ...
          <div className="lg:col-span-1 rounded-[28px] border border-border bg-surface p-6 shadow-xl lg:sticky top-6 flex flex-col gap-6">
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div className="space-y-1.5 px-2">
                <h3 className="text-2xl font-bold text-text">{currentGroup.name}</h3>
